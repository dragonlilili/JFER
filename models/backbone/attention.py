...
def multi_head_attention_forward(
    ...
    if not need_weights and hasattr(F, 'scaled_dot_product_attention'):
        ...
        v = v.contiguous().view(bsz, num_heads, src_len, v_head_dim)
        if attn_mask is not None:
            if attn_mask.size(0) == 1:
                attn_mask = attn_mask[:, None, :, :]
            else:
                attn_mask = attn_mask.contiguous().view(bsz, num_heads, attn_mask.size(-2), attn_mask.size(-1))

        attn_output = F.scaled_dot_product_attention(q, k, v, attn_mask=attn_mask, dropout_p=dropout_p)
        attn_output = attn_output.permute(2, 0, 1, 3).contiguous().view(tgt_len, bsz, vdim)
