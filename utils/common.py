"""Distributed training, logging, and checkpoint utilities."""
...
def rotate_points_along_z(points, angle):
    ...
    view_shape = [points.shape[0]] + [1] * (points.dim() - 2)
    cosa = cosa.view(*view_shape)
    sina = sina.view(*view_shape)
    ...
    y_rot = x * sina + y * cosa
    if points.shape[-1] == 2:
        points_rot = torch.stack((x_rot, y_rot), dim=-1)
    else:
        points_rot = torch.stack((x_rot, y_rot, points[..., 2]), dim=-1)
