...
from data.base_dataset import DatasetTemplate
...
class WaymoDataset(DatasetTemplate):
    ...
    def get_all_infos(self, info_path):
        ...
        sample_interval = int(
            self.dataset_cfg.SAMPLE_INTERVAL[self.mode]
        )
        sample_offset_cfg = self.dataset_cfg.get("SAMPLE_OFFSET", {})
        if hasattr(sample_offset_cfg, "get"):
            sample_offset = int(sample_offset_cfg.get(self.mode, 0))
        else:
            sample_offset = int(sample_offset_cfg)
        if sample_interval <= 0:
            raise ValueError("SAMPLE_INTERVAL must be positive")
        if not 0 <= sample_offset < sample_interval:
            raise ValueError(
                "SAMPLE_OFFSET must be in [0, SAMPLE_INTERVAL), got "
                f"offset={sample_offset}, interval={sample_interval}"
            )
        infos = src_infos[sample_offset::sample_interval]
        self.logger.info(
            "Sampling scenes with interval=%d, offset=%d",
            sample_interval,
            sample_offset,
        )
    ...
    def generate_centered_trajs_for_agents(self, center_objects, obj_trajs_past, obj_types, center_indices, sdc_index, timestamps, obj_trajs_future):
        ...
        type_embedding = self.dataset_cfg.get('AGENT_TYPE_EMBEDDING', None)
        if type_embedding is None:
            # Preserve the base feature layout for existing Waymo/AV2
            # checkpoints. New AV2-native configs opt in explicitly.
            type_embedding = ['TYPE_VEHICLE', 'TYPE_PEDESTRAIN', 'TYPE_CYCLIST']
        type_embedding = list(type_embedding)
        object_onehot_mask = torch.zeros((num_center_objects, num_objects, num_timestamps, len(type_embedding) + 2))
        for type_idx, type_name in enumerate(type_embedding):
            object_onehot_mask[:, obj_types == type_name, :, type_idx] = 1
        object_onehot_mask[torch.arange(num_center_objects), center_indices, :, len(type_embedding)] = 1
        object_onehot_mask[:, sdc_index, :, len(type_embedding) + 1] = 1
    ...
    def create_map_data_for_center_objects(
        ...
        map_infos,
        center_offset,
        return_selected_chunk_ids=False,
    ):
        """
        Args:
            center_objects (num_center_objects, 10): [cx, cy, cz, dx, dy, dz, heading, vel_x, vel_y, valid]
        ...
        if len(batch_polylines) > num_of_src_polylines:
            ...
            selected_chunk_ids = torch.arange(
                len(batch_polylines), dtype=torch.long
            )[None, :].repeat(num_center_objects, 1)
        ...
        if return_selected_chunk_ids:
            return (
                map_polylines,
                map_polylines_mask,
                map_polylines_center,
                selected_chunk_ids.numpy(),
            )
