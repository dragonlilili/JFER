...
def transform_preds_to_waymo_format(pred_dicts, top_k_for_eval=-1, eval_second=8):
    ...
    gt_is_valid = np.zeros((num_scenario, num_max_objs_per_scene, num_frames_in_total), dtype=bool)
    pred_gt_idxs = np.zeros((num_scenario, num_max_objs_per_scene, 1), dtype=np.int64)
    pred_gt_idx_valid_mask = np.zeros((num_scenario, num_max_objs_per_scene, 1), dtype=bool)
    object_type = np.zeros((num_scenario, num_max_objs_per_scene), dtype=np.int64)
    object_id = np.zeros((num_scenario, num_max_objs_per_scene), dtype=object)
